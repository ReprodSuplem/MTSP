...
def insGen(fileName, nofSManStr)
  ...
  uMatx = Array.new((0..nofCity-2).to_a).map{
...
=end
  ...
  for i in 1..nofCity-1
    for j in 1..nofCity-1
      if j != i
        tmpHS = Hash.new();
        for k in 1..nofCity-2
          tmpHS.store(uMatx[i-1][k], k);
          tmpHS.store(-1*(uMatx[j-1][k]).to_i, k);
        end
        for k in 0..nofSMan-1
          tmpHS.store(pMatx[k][i][j], tmpInt);
        end
        hashArray.push(tmpHS);
      end
    end
  end
  ...
  for i in 0..hashArray.size-1
    $satFormStr << "#{$sumOfWeights+1} -#{totalizer(hashArray[i])[tmpInt + (2..nofCity-2).to_a.sum]} 0\n";
    $nofClause += 1;
  end
